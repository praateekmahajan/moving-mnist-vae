...
def test(model, data_loader, optimizer, device, is_255=True, is_mnist=True, pixelcnn=False, num_images=6,
         target_num_images=10, imsize=64, mean=0.1307, std=0.3081):
    time_tr = time.time()
    model.eval()
    ...
    normal_z = torch.distributions.Normal(0, 1).sample(
        (input_for_plots.shape[0], encoding.shape[1], encoding.shape[2], encoding.shape[3])).to(device)
    ...
    if pixelcnn:
        ...
        normal_reconstruction_without_tf, normal_stacked_without_tf = generate(normal_z_image, \
                                                                               torch.zeros(normal_z_image.shape).to(
                                                                                   device), \
    ...
    if is_255:
        out_z_sliding, stacked_z_sliding = generate(z_images, torch.zeros(z_images.shape).to(device), model, mean, std,
                                                    imsize)
        reconstruction = model.get_reconstruction(movement_sliding_z,
                                                  torch.zeros((movement_sliding_z.shape[0], 1, imsize, imsize)).to(
                                                      device))
...
def get_dataset(dataset, folder, transform=None, target_transform=None):
    assert(dataset in ['MNIST', 'MovingMNIST'])
    if dataset == "MNIST":
        return MNIST(folder, train=True, transform=transform, target_transform=target_transform, download=True), \
               MNIST(folder, train=False, transform=transform, target_transform=target_transform, download=True),

    return MovingMNISTDataset(folder, train=True, transform=transform, target_transform=target_transform), \
           MovingMNISTDataset(folder, train=False, transform=transform, target_transform=target_transform),


def save_kmeans_file(n_clusters, dataset="MovingMNIST", folder="data"):
    train_dataset, _ = get_dataset(dataset, folder)

    numpy_train_data = train_dataset.train_data if dataset=="MovingMNIST" else train_dataset.train_data.numpy()
